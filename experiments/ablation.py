...
Output: ``experiments/results/ablation.csv`` (one row per amplitude x condition)
and a printed matrix.
"""
...
def condition_nodes(condition):
    """Indices removed by a condition. Empty type strings match nothing."""
...
def main(argv=None):
    ...
    head = f"{'condition':<10}{'removed':>8}  " + "".join(f"{a:>14.2f}" for a in amps)
    print(head)
    print("-" * len(head))
    for c in CONDITIONS:
        line = f"{c:<10}{len(nodes_by_cond[c]):>8}  "
        for a in amps:
